...
int main(int argc, char *argv[]) {
    ...
    int connection_res = connect(sockfd, (struct sockaddr*)&s_addr, sizeof(s_addr));
    if(connection_res != 0){
    ...
    int send_res = send(sockfd, message, mess_size, 0);
    printf("Bytes sent: %d\n\n", send_res);
    ...
    int res_form = read(sockfd, buf, MAX);
    ...
    //sinon on affiche le message et on finit le programme
        ...
        printf("Got an answer, closing the connection!");
    }
    close(sockfd);
    return 0;
}
